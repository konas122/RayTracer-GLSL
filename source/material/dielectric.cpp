...
std::optional<BSDFSample> DielectricMaterial::sampleBSDF(const glm::vec3 &hit_point, const glm::vec3 &view_direction, const RNG &rng) const {
    ...
    if (rng.uniform() <= fr) {
        ...
        glm::vec3 brdf = albedo_r * microfacet.normalDistribution(microfacet_normal)
        ...
        float pdf = microfacet.visibleNormalDistribution(view_direction, microfacet_normal) / glm::abs(4.f * glm::dot(view_direction, microfacet_normal));
        ...
        glm::vec3 btdf = albedo_t * det_J * microfacet.normalDistribution(microfacet_normal)
            * microfacet.heightCorrelatedMaskingShadowing(light_direction, view_direction, microfacet_normal)
            * glm::abs(glm::dot(view_direction, microfacet_normal) / (light_direction.y * view_direction.y));
        float pdf = microfacet.visibleNormalDistribution(view_direction, microfacet_normal) * det_J;
