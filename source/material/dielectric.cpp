...
static float Fresnel(float etai_div_etat, float cos_theta_t, float &cos_theta_i) {
...
std::optional<BSDFSample> DielectricMaterial::sampleBSDF(const glm::vec3 &hit_point, const glm::vec3 &view_direction, const RNG &rng) const {
    ...
    if (rng.uniform() <= fr) {
        ...
        glm::vec3 brdf = fr * albedo_r * microfacet.normalDistribution(microfacet_normal)
        ...
        float pdf = fr * microfacet.visibleNormalDistribution(view_direction, microfacet_normal) / glm::abs(4.f * glm::dot(view_direction, microfacet_normal));
        ...
        glm::vec3 btdf = (1.f - fr) * albedo_t * det_J * microfacet.normalDistribution(microfacet_normal)
            * microfacet.heightCorrelatedMaskingShadowing(light_direction, view_direction, microfacet_normal)
            * glm::abs(glm::dot(view_direction, microfacet_normal) / (light_direction.y * view_direction.y));
        float pdf = (1.f - fr) * microfacet.visibleNormalDistribution(view_direction, microfacet_normal) * det_J;
...
glm::vec3 DielectricMaterial::BSDF(const glm::vec3 &hit_point, const glm::vec3 &light_direction, const glm::vec3 &view_direction) const {
    ...
    if (lv < 0) {
        ...
        glm::vec3 btdf = (1 - fr) * albedo_t * det_J * microfacet.normalDistribution(microfacet_normal)
            * microfacet.heightCorrelatedMaskingShadowing(light_direction, view_direction, microfacet_normal)
...
float DielectricMaterial::PDF(const glm::vec3 &hit_point, const glm::vec3 &light_direction, const glm::vec3 &view_direction) const {
    if (isDeltaDistribution()) {
        return 0;
    }
    ...
    if (lv < 0) {
        ...
        float det_J =
            ...
            glm::abs(glm::dot(light_direction, microfacet_normal)) /
            glm::pow(
                glm::abs(glm::dot(view_direction, microfacet_normal)) -
                    etai_div_etat * etai_div_etat *
                        glm::abs(glm::dot(light_direction, microfacet_normal)),
                2
            );
        return (1.f - fr) * microfacet.visibleNormalDistribution(view_direction, microfacet_normal) * det_J;
    }
    glm::vec3 microfacet_normal = glm::normalize(light_direction + view_direction);
    if (microfacet_normal.y < 0) {
        microfacet_normal = -microfacet_normal;
    }
    return fr * microfacet.visibleNormalDistribution(view_direction, microfacet_normal) / glm::abs(4.f * glm::dot(view_direction, microfacet_normal));
}
