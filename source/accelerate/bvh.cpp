...
void BVH::build(std::vector<Triangle> &&triangles) {
    ...
    for (const auto &triangle : ordered_triangles) {
        areas.emplace_back(triangle.getArea());
        area += areas.back();
    }
...
void BVH::recursiveSplit(BVHTreeNode *node, BVHState &state) {
    ...
    for (size_t axis = 0; axis < 3; axis ++) {
        ...
        for (const auto &triangle : node->triangles) {
            ...
            triangle_indices_buckets[axis][bucket_idx].emplace_back(triangle_idx);
            triangle_idx ++;
        }
    ...
    for (size_t i = 0; i < min_split_index; i ++) {
        for (size_t idx : triangle_indices_buckets[node->split_axis][i]) {
            node->left->triangles.emplace_back(node->triangles[idx]);
        }
    }
    for (size_t i = min_split_index; i < bucket_count; i ++) {
        for (size_t idx : triangle_indices_buckets[node->split_axis][i]) {
            node->right->triangles.emplace_back(node->triangles[idx]);
        }
    }
...
size_t BVH::recursiveFlatten(BVHTreeNode *node) {
    ...
    if (bvh_node.triangle_count == 0) {
        ...
        std::copy(node->triangles.cbegin(), node->triangles.cend(), std::back_inserter(ordered_triangles));
    }
    return idx;
}
...
std::optional<ShapeSample> BVH::sampleShape(const RNG &rng) const {
    ...
    return ShapeSample {
        triangle_sample->point,
        triangle_sample->normal,
        triangle_sample->pdf * sample_result.prob
    };
}
