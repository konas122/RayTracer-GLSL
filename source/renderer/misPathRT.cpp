...
static float PowerHeuristic(float pdf_j, float pdf_k) {
    return pdf_j * pdf_j / (pdf_j * pdf_j + pdf_k * pdf_k);
}

glm::vec3 MISPathTracingRenderer::renderPixel(const glm::ivec3 &pixel_coord) {
    RNG rng {};
    ...
    while (true) {
        ...
        if (hit_info.has_value()) {
            if (hit_info->material && hit_info->material->area_light) {
                float weight_bsdf = 1;
                if (!last_is_specular) {
                    float light_source_prob = scene.getLightSampler().getProb(hit_info->material->area_light, allow_mis_compensation);
            ...
            if (hit_info->material) {
                ...
                if (!last_is_specular) {
                    auto light_source_sample = scene.getLightSampler().sample(rng.uniform(), allow_mis_compensation);
            ...
            if (last_is_specular) {
                ...
                for (const auto *infinite_light : scene.getInfiniteLights()) {
                    float light_source_prob = scene.getLightSampler().getProb(infinite_light, allow_mis_compensation);
