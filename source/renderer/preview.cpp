...
Previewer::Previewer(BaseRenderer &base_renderer, float fps) : base_renderer(base_renderer), fps(fps) {
    ...
    render_modes.emplace_back(&base_renderer);
    render_modes.emplace_back(new NormalRenderer(base_renderer.camera, base_renderer.scene));
    DEBUG_LINE(render_modes.emplace_back(new BoundsTestCountRenderer(base_renderer.camera, base_renderer.scene)));
    DEBUG_LINE(render_modes.emplace_back(new TriangleTestCountRenderer(base_renderer.camera, base_renderer.scene)));
...
void Previewer::renderFrame() {
    ...
    ThreadPool::getThreadPool()->parallelFor(film.getWidth(), film.getHeight(), [&](size_t x, size_t y) {
