            ...
            if (hit_info->material) {
                ...
                if (!last_is_specular) {
                    ...
                    if (light_source_sample.has_value()) {
                      auto light_sample =
                          light_source_sample->light->sampleLight(
                              hit_info->hit_point, scene.getRadius(), rng,
                              false
                        );
                        if (
                            light_sample.has_value() &&
                            (!scene.intersect(
                                {
                                    hit_info->hit_point,
                                    light_sample->light_point - hit_info->hit_point
                                },
                                1e-5, 1.f - 1e-5)
                            )
                        ) {
                            glm::vec3 light_direction_local = frame.localFromWorld(light_sample->light_direction);
                            L += beta * hit_info->material->BSDF(
                                hit_info->hit_point,
                                light_direction_local,
                                view_direction)
                                * glm::abs(light_direction_local.y)
                                * light_sample->Le
                                / (light_sample->pdf * light_source_sample->prob);
                        }
                    }
                }

                auto bsdf_sample = hit_info->material->sampleBSDF(
                    hit_info->hit_point, view_direction, rng);
